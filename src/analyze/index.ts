...
export const analyzeCodebase = async (options: IAnalysisOptions): Promise<void> => {
    ...
    console.log(chalk.bold(`\n\nAnalyzing codebase in ${chalk.cyan(options.directory)} ${options?.framework ? `framework: ${chalk.cyan(options.framework)}` : ""}\n`));
    ...
    await traverseDirectory({
        ...
        onFile: (filePath) => {
            ...
            //@ts-ignore
            if (fileNameCases[fileNameCase] !== undefined) fileNameCases[fileNameCase] += 1;
